...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map((num: string): number => (+num ? +num : 0));
}
...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts
        ...
        .map((num: string): number => (Number.isNaN(+num) ? 0 : +num));
};
...
export function injectPositive(values: number[]): number[] {
    ...
    const clonedValues1 = [...values];
    const clonedValues2 = [...values];
    clonedValues1.splice(
        firstNegativeIndex,
        clonedValues1.length - firstNegativeIndex
    );
    const sum = clonedValues1.reduce(
        (sum: number, value: number) => sum + value,
        0
    );
    clonedValues2.splice(firstNegativeIndex + 1, 0, sum);
    return clonedValues2;
}
